...
export const getEbooks = unstable_cache(
    async () => {
        try {
            ...
            console.error("[getEbooks] Error fetching ebooks:", error);
            return [];
        }
    },
    ["all-ebooks-v7"],
    ...
    unstable_cache(
        async () => {
            try {
                ...
                console.error(`[getEbooksByLanguage] Error fetching ebooks for ${language}:`, error);
                return [];
            }
        },
...
export const getComboEbooks = unstable_cache(
    async () => {
        try {
            ...
            console.error("[getComboEbooks] Error fetching combo ebooks:", error);
            return [];
        }
    },
    ["combo-ebooks-v7"],
    ...
    return unstable_cache(
        async (id: string) => {
            try {
                ...
                let sortedIncludedEbooks = (ebook.includedEbooks || [])
                    .map((ci) => ci?.ebook)
                ...
                console.error(`[getEbookById] Error fetching ebook id=${id}:`, error);
                return null;
            }
        },
        [`ebook-${id}-sorted-v3`],
